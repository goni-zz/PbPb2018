...
class HLTObjectMonitorHI : public DQMEDAnalyzer {
  ...
  //declare params
  //2018 PbPb run
  edm::ParameterSet jet1Pt_pset;
  edm::ParameterSet jet1Eta_pset;
  edm::ParameterSet jet1Phi_pset;
...

{
  ...
  jet1Pt_pset       = iConfig.getParameter<edm::ParameterSet>("jet1Pt");
  plotMap[&jet1Pt_] = &jet1Pt_pset;
  jet1Eta_pset      = iConfig.getParameter<edm::ParameterSet>("jet1Eta");
  plotMap[&jet1Eta_] = &jet1Eta_pset;
  jet1Phi_pset      = iConfig.getParameter<edm::ParameterSet>("jet1Phi");
  plotMap[&jet1Phi_] = &jet1Phi_pset;
  ...
  HIFullTracksMultiplicity6080Pt_pset = iConfig.getParameter<edm::ParameterSet>("HIFullTracksMultiplicity6080Pt");
  plotMap[&HIFullTracksMultiplicity6080Pt_] = &HIFullTracksMultiplicity6080Pt_pset;
  HIFullTracksMultiplicity6080Eta_pset = iConfig.getParameter<edm::ParameterSet>("HIFullTracksMultiplicity6080Eta");
  ...
  using namespace edm;
   ...
   for (auto plot: plotList) //loop over paths
       ...
       else if(plot->pathIndexOR > 0 && triggerResults->accept(plot->pathIndexOR) && hltConfig_.saveTags(plot->moduleNameOR))
	 {
	   ...
	   //if (pathName == jet1Pt_.pathName){
	   //   for (const auto & key : keys){
	   //      jet1Pt_.ME->Fill(objects[key].pt());
	   //      jet1Eta_.ME->Fill(objects[key].eta());
	   //      jet1Phi_.ME->Fill(objects[key].phi());
	   //   }
	   //}
	   ////cspf jet pt + eta + phi
	   if (pathName == HICSPFJet60Pt_.pathName){
	      for (const auto & key : keys){
	   ...
	   else if (pathName == HIFullTracksMultiplicity6080Pt_.pathName){
	      for (const auto & key : keys){
		 HIFullTracksMultiplicity6080Pt_.ME->Fill(objects[key].pt());
		 HIFullTracksMultiplicity6080Eta_.ME->Fill(objects[key].eta());
		 HIFullTracksMultiplicity6080Phi_.ME->Fill(objects[key].phi());
	      }
	   }
	   else if (pathName == HIDoubleMu0HFVetoInAndMaxTrackPt_.pathName){
	      for (const auto & key : keys){
		 HIDoubleMu0HFVetoInAndMaxTrackPt_.ME->Fill(objects[key].pt());
		 HIDoubleMu0HFVetoInAndMaxTrackEta_.ME->Fill(objects[key].eta());
		 HIDoubleMu0HFVetoInAndMaxTrackMass_.ME->Fill(objects[key].mass());
	      }
	   }
           else if (pathName == jet1Pt_.pathName){
	      for (const auto & key : keys){
	         jet1Pt_.ME->Fill(objects[key].pt());
	         jet1Eta_.ME->Fill(objects[key].eta());
	         jet1Phi_.ME->Fill(objects[key].phi());
               ...
               for (const auto & key1: keys){
                 ...
                 for (const auto & key2: keys){
                   if (key0 != key1 && kCnt1 > kCnt0 && key0 != key2 && key1 != key2 && kCnt2 > kCnt1){ // avoid filling hists with same objs && avoid double counting separate objs
                     ...
                     if(dimu.Phi()>HITktkDzeroPhi_.xMin && dimu.Phi()<HITktkDzeroPhi_.xMax) HITktkDzeroPhi_.ME->Fill(dimu.Phi());
               ...
               for (const auto & key1: keys){
                 ...
                 for (const auto & key2: keys){
                   if (key0 != key1 && kCnt1 > kCnt0 && key0 != key2 && key1 != key2 && kCnt2 > kCnt1){ // avoid filling hists with same objs && avoid double counting separate objs
                     ...
                     if(dimu.Phi()>HITktktkDsPhi_.xMin && dimu.Phi()<HITktktkDsPhi_.xMax) HITktktkDsPhi_.ME->Fill(dimu.Phi());
               ...
               for (const auto & key1: keys){
                 ...
                 for (const auto & key2: keys){
                   if (key0 != key1 && kCnt1 > kCnt0 && key0 != key2 && key1 != key2 && kCnt2 > kCnt1){ // avoid filling hists with same objs && avoid double counting separate objs
                     ...
                     if(dimu.Phi()>HITktktkLcPhi_.xMin && dimu.Phi()<HITktktkLcPhi_.xMax) HITktktkLcPhi_.ME->Fill(dimu.Phi());
