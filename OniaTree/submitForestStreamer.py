...
parser.add_option('-q', '--queue'      ,dest='queue'  ,help='batch queue'          ,default='1nd')#cmscaf1nd(1 day), 8nm(8 min), 1nh(1 hour), 1nw(1 week), 2nd(2 days)
...
parser.add_option('-o', '--output'     ,dest='output' ,help='output directory', default='/eos/cms/store/group/phys_heavyions/dileptons/Data2018/PbPb502TeV/TTrees/test')
...
#            scriptFile.write('cp $CMSSW_BASE/src/HeavyIonsAnalysis/JetAnalysis/test/*.db .\n')
	    scriptFile.write('cmsRun %s outputFile=step3_%d.root maxEvents=-1 inputFiles=%s\n' % (recoCfg,jobCounter,line) )
            ...
            scriptFile.write('eval `scram r -sh`\n')
            scriptFile.write('cd -\n')
	    scriptFile.write('cmsRun %s outputFile=HiOniatree_%d.root maxEvents=-1 inputFiles=file:step3_%d.root\n' % (OniaCfg,jobCounter,jobCounter) )
            ...
            #scriptFile.write('eos cp HiOniatree_%d.root root://eoscms//eos/cms%s/HiOniatree_%d.root\n' % (jobCounter,outdir3,jobCounter) )
            ...
            if opt.queue=='':
                ...
                os.system("echo bsub -q %s -R \"swp>1000 && pool>30000\" -J HIONIA_%d \'%s/runJob_%d.sh\'" % (opt.queue,jobCounter,jobsBase,jobCounter) )
                os.system("bsub -q %s -N -u null@null -R \"swp>1000 && pool>30000\" -J HIONIA_%d \'%s/runJob_%d.sh\'" % (opt.queue,jobCounter,jobsBase,jobCounter) )
